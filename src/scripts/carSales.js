...
salesByWeek.forEach(sale => {
    reportContainer.innerHTML += `<h1>${sale.sales_agent.first_name} ${sale.sales_agent.last_name}</h1>`;
    for (let entry of Object.entries(sale.vehicle)) {
        reportContainer.innerHTML += `<p>${entry[0]}: ${entry[1]}</p>`;
    }

    reportContainer.innerHTML += `<h3>Gross Profit: ${sale.gross_profit}`;
});